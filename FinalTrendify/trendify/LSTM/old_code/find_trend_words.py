...
# def load_frequency_time_series_csv(csv_file_path="../frequency_time_series_data.csv"):
...
def fetch_dataset(csv_file_path):
...
def main():
    ...
    # Get the frequency data and word mapping dict.
    word_frequency_data, word_mapping_idx = fetch_dataset(args.csv_file_path)
    ...
    trend_df = pd.DataFrame(
        data, columns=["Word", "# Anomaly (Threshold 1.2)", "# Anomaly (Threshold 1.5)", "# Anomaly (Threshold 1.8)"])
    trend_df.to_csv('../results/trending_list.csv')
