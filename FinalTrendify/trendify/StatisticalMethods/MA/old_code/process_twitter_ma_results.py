...
def analyze_trend_points():
    ...
    df = pd.read_csv(os.path.join(OUTPUT_DIR, 'twitter_data_ma_0_1374.csv'))
    ...
    ans.to_csv(os.path.join(OUTPUT_DIR, "twitter_ma_trend_list.csv"),index=False)
...
def compare_with_ebay():
    ...
    a_df = pd.read_csv(os.path.join(OUTPUT_DIR, 'twitter_ma_trend_list.csv'))
    ...
    plus_5 = ab_df[(ab_df['s_trend_5'] == True) & (ab_df['ebay_trend'] == True)].shape[0] / ab_df[ab_df['ebay_trend'] == True].shape[0] * 100
    plus_6 = ab_df[(ab_df['s_trend_6'] == True) & (ab_df['ebay_trend'] == True)].shape[0] / ab_df[ab_df['ebay_trend'] == True].shape[0] * 100
    plus_7 = ab_df[(ab_df['s_trend_7'] == True) & (ab_df['ebay_trend'] == True)].shape[0] / ab_df[ab_df['ebay_trend'] == True].shape[0] * 100
    plus_8 = ab_df[(ab_df['s_trend_8'] == True) & (ab_df['ebay_trend'] == True)].shape[0] / ab_df[ab_df['ebay_trend'] == True].shape[0] * 100
    plus_9 = ab_df[(ab_df['s_trend_9'] == True) & (ab_df['ebay_trend'] == True)].shape[0] / ab_df[ab_df['ebay_trend'] == True].shape[0] * 100
    plus_10 = ab_df[(ab_df['s_trend_10'] == True) & (ab_df['ebay_trend'] == True)].shape[0] / ab_df[ab_df['ebay_trend'] == True].shape[0] * 100
    


    ab_df.drop(labels=['point_counts', 's_trend_5', 's_trend_6', 's_trend_7', 's_trend_9', 's_trend_10'], axis=1, inplace=True)
    print(ab_df.columns)
    ab_df.columns = ['words', 'ebay_trend', 'ma_trend']
    print(ab_df.columns)
    ab_df.to_csv(os.path.join(OUTPUT_DIR, 'twitter_ebay_ma_processed.csv'), index=False)
