...
def combine_wiki_sarima_multi_process_results(directory_path, pair_wise_index_list):
    csv_files = [f'wiki_data_sarima_{start}_{end}.csv' for start,end in pair_wise_index_list]
    ...
    df.to_csv(os.path.join(OUTPUT_DIR, 'wiki_data_sarima_0_41180.csv'), index=False)
    # print(df.shape)
    # print(df.head(10))
...
combine_wiki_sarima_multi_process_results(OUTPUT_DIR, get_pair_wise_indices())


def analyze_trend_points():

    df = pd.read_csv(os.path.join(OUTPUT_DIR, 'wiki_data_sarima_0_41180.csv'))
    ...
    ans.to_csv(os.path.join(OUTPUT_DIR, "wiki_sarima_trend_list.csv"),index=False)
...
def compare_with_ebay():
    ...
    s_df = pd.read_csv(os.path.join(OUTPUT_DIR, 'wiki_sarima_trend_list.csv'))
    ...
    es_df.drop(labels=['point_counts', 's_trend_8', 's_trend_10', 's_trend_11', 's_trend_12'], axis=1, inplace=True)
    print(es_df.columns)
    es_df.columns = ['words', 'ebay_trend', 'sarima_trend']
    print(es_df.columns)
    es_df.to_csv(os.path.join(OUTPUT_DIR, 'wiki_ebay_sarima_processed.csv'), index=False)
