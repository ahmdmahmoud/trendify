...
def get_sarima_model_rmse(data, p_d_q, test_split, diff_interval):
    ...
    p_p,p_d,p_q,p_s = p_d_q
    p_d_q = (p_p,p_d,p_q)

    model = sm.tsa.statespace.SARIMAX(diff_history, order=p_d_q, seasonal_order=(0,0,0,p_s), enforce_stationarity=False, enforce_invertibility=False)
    model_fit = model.fit()
...
def find_best_rmse_SARIMA_model(scaled_freq_time_series, words, start, end, test_split, p_d_q_permutation, diff_interval):
    RMSE, anomaly_points = [], []
    for word in words[start : end]:
        ...
        for p_d_q in p_d_q_permutation:
            rmse, anomaly_counts = get_sarima_model_rmse(data, p_d_q, test_split, diff_interval) 
...
def calculate_sarima(*args):
    ...
    p_d_q_permutation = list(itertools.product(p_p,p_d,p_q,p_s))

    calculation_df = find_best_rmse_SARIMA_model(scaled_freq_time_series, words, start, end, test_split, p_d_q_permutation, repeat_interval)

    fpath = os.path.join(RESULT_DIR, f'result_sarima_{start}_{end}.csv')
