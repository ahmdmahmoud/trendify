...
def analyze_trend_points():
    df = pd.read_csv(os.path.join(OUTPUT_DIR, 'twitter_data_tes_0_1374.csv'))
    ...
    ans.to_csv(os.path.join(OUTPUT_DIR, "twitter_tes_trend_list.csv"),index=False)
...
def compare_with_ebay():
    ...
    s_df = pd.read_csv(os.path.join(OUTPUT_DIR, 'twitter_tes_trend_list.csv'))
    ...
    print(f'{es_df[(es_df["s_trend_8"] == True) & (es_df["ebay_trend"] == True)].shape[0]}')
    print(plus_5, plus_6, plus_7, plus_8, plus_9, plus_10)


    es_df.drop(labels=['point_counts', 's_trend_5', 's_trend_6', 's_trend_7', 's_trend_9', 's_trend_10'], axis=1, inplace=True)
    print(es_df.columns)
    es_df.columns = ['words', 'ebay_trend',  'tes_trend']
    print(es_df.columns)
    es_df.to_csv(os.path.join(OUTPUT_DIR, 'twitter_ebay_tes_processed.csv'), index=False)
